...
def train_and_calc_inf_network(train_sample_nr, current_layer_size, repeat_nr, layerSize, num_of_ephocs, learning_rate_local, batch_size, epoch_indexes, save_grads, data_sets_org,
    ...
    if calc_information:
        ...
        network['information'] = np.array([inn.get_information(network['ws'], data_sets_org.data, data_sets_org.labels, num_of_bins, interval_information_display, network['model'], layerSize)])
    elif calc_information_last:
        print('Calculating the infomration for the last epoch')
        network['information'] = np.array([inn.get_information([network['ws'][-1]], data_sets_org.data, data_sets_org.labels, num_of_bins, interval_information_display, network['model'], layerSize)])
